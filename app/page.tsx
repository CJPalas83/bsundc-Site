...
function CollectionsMobile() {
  ...
  // 3 tiles total -> slide from 0vw to -200vw (last card lines up at end)
  const x = useTransform(scrollYProgress, [0, 1], ["0vw", "-200vw"]);
  // Pagination dot opacities (hooks must be unconditional + outside loops).
  const dot0Opacity = useTransform(scrollYProgress, [0, 0.25, 0.5], [1, 0.2, 0.2]);
  const dot1Opacity = useTransform(scrollYProgress, [0.25, 0.5, 0.75], [0.2, 1, 0.2]);
  const dot2Opacity = useTransform(scrollYProgress, [0.5, 0.75, 1], [0.2, 0.2, 1]);
        ...
        <div className="flex-1 overflow-hidden">
          <motion.div className="flex h-full" style={{ x }}>
            {featuredCollections.map((c) => (
              <div
                key={c.href}
                className="w-screen shrink-0 px-6 pb-5 flex flex-col gap-3"
              >
                <Link
                  href={c.href}
                ...
                >
                  <div className="relative flex-1 bg-bg-feature overflow-hidden">
                    <Image
                      src={c.image}
                      alt={c.title}
                      fill
            ...
            {/* Placeholder slot */}
            <div className="w-screen shrink-0 px-6 pb-5 flex flex-col gap-3">
              <div className="flex-1 flex flex-col bg-bg-main border border-text-main/5 overflow-hidden">
                <div className="relative flex-1 bg-bg-feature flex flex-col items-center justify-center gap-3">
...
function WhoWeServeMobile() {
  ...
  // 4 tiles -> slide from 0vw to -300vw
  const x = useTransform(scrollYProgress, [0, 1], ["0vw", "-300vw"]);
  // Pagination dot opacities (hooks must be unconditional + outside loops).
  const dot0Opacity = useTransform(scrollYProgress, [0, 1 / 6, 1 / 3], [1, 0.2, 0.2]);
  const dot1Opacity = useTransform(scrollYProgress, [1 / 6, 1 / 3, 1 / 2], [0.2, 1, 0.2]);
  const dot2Opacity = useTransform(scrollYProgress, [1 / 3, 2 / 3, 5 / 6], [0.2, 1, 0.2]);
  const dot3Opacity = useTransform(scrollYProgress, [2 / 3, 5 / 6, 1], [0.2, 0.2, 1]);
